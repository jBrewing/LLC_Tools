...
agg = {'coldInFlowRate':'first',
          'hotInFlowRate':'first',# take first obs to simulate a lower sampling frequency
...
df_30s[['coldInFlowRate','hotInFlowRate']] = df_30s[['coldInFlowRate', 'hotInFlowRate']] * 30 # multiply first obs by period value represents
df_1t[['coldInFlowRate','hotInFlowRate']] = df_1t[['coldInFlowRate', 'hotInFlowRate']] * 60 # i.e. a 1 sec obs representing 1 min = obs * 60s
df_5t[['coldInFlowRate','hotInFlowRate']] = df_5t[['coldInFlowRate', 'hotInFlowRate']] * 300
df_30t[['coldInFlowRate','hotInFlowRate']] = df_30t[['coldInFlowRate', 'hotInFlowRate']] * 1800
df_1h[['coldInFlowRate','hotInFlowRate']] = df_1h[['coldInFlowRate', 'hotInFlowRate']] * 3600
...
df_raw = df_raw.cumsum() # .cumsum() to cumulatively sum water-related energy use
...
fig.suptitle('SAMPLE INTERVAL', fontsize = 18)

fig.autofmt_xdate()

ax.plot(df_raw['hotUse_energy'], color='red', label='Pulse Aggregated Data')
ax.plot(df_30s['hotUse_energy'], color='blue', label = '30sec Sample Interval')
ax.plot(df_1t['hotUse_energy'], color='green', label = '1min Sample Interval')
ax.plot(df_5t['hotUse_energy'], color='orange', label = '5min Sample Interval')
ax.plot(df_30t['hotUse_energy'], color='black', label = '30min Sample Interval')
ax.plot(df_1h['hotUse_energy'], color='purple', label = '1h Sample Interval')
...
fig2.suptitle('SAMPLE INTERVAL', fontsize = 18)
fig2.autofmt_xdate()

ax2.plot(df_raw['hotUse_energy'], color='red', label='Pulse-Aggregated Data')
ax2.plot(df_30s['hotUse_energy'], color='blue', label = '30sec Sample Interval')
ax2.plot(df_1t['hotUse_energy'], color='green', label = '1min Sample Interval')
ax2.plot(df_5t['hotUse_energy'], color='orange', label = '5min Sample Interval')
ax2.plot(df_30t['hotUse_energy'], color='black', label = '30min Sample Interval')
ax2.plot(df_1h['hotUse_energy'], color='purple', label = '1h Sample Interval')

ax2.legend(fontsize=14, loc=2)
ax2.grid(True)
ax2.set_xlim("'2019-03-24T00:00:00Z'", "'2019-03-25T00:00:00Z'")
ax2.set_ylim(0,1200)
